  ...
  for(m in 1:length(modules)){
    ...
    if (cancertype == "THCA") {
      if (length(summary(module2trait$nested.batch))>1) {
        mod0 <- lm(L1HS ~ garbage + oldLINE, data=module2trait)
        mod1 <- lm(L1HS ~ log2TPMsum + oldLINE, data=module2trait)
        mod2 <- lm(L1HS ~ modulePC1 + oldLINE, data=module2trait)
        mod3 <- lm(L1HS ~ modulePC1 + log2TPMsum + oldLINE, data=module2trait)
        mod4 <- lm(L1HS ~ nested.batch + oldLINE, data=module2trait)
        mod5 <- lm(L1HS ~ log2TPMsum + nested.batch + oldLINE, data=module2trait)
        mod6 <- lm(L1HS ~ modulePC1 + nested.batch + oldLINE, data=module2trait)
        mod7 <- lm(L1HS ~ modulePC1 + log2TPMsum + nested.batch + oldLINE, data=module2trait)
        mod8 <- lm(L1HS ~ radiation + oldLINE, data=module2trait)
        mod9 <- lm(L1HS ~ log2TPMsum + radiation + oldLINE, data=module2trait)
        mod10 <- lm(L1HS ~ modulePC1 + radiation + oldLINE, data=module2trait)
        mod11 <- lm(L1HS ~ modulePC1 + log2TPMsum + radiation + oldLINE, data=module2trait)
        mod12 <- lm(L1HS ~ nested.batch + radiation + oldLINE, data=module2trait)
        mod13 <- lm(L1HS ~ log2TPMsum + nested.batch + radiation + oldLINE, data=module2trait)
        mod14 <- lm(L1HS ~ modulePC1 + nested.batch + radiation + oldLINE, data=module2trait)
        mod15 <- lm(L1HS ~ modulePC1 + log2TPMsum + nested.batch + radiation + oldLINE, data=module2trait)
        ...
        mod2 <- lm(L1HS ~ modulePC1 + oldLINE, data=module2trait)
        mod3 <- lm(L1HS ~ modulePC1 + log2TPMsum + oldLINE, data=module2trait)
        mod4 <- lm(L1HS ~ radiation + oldLINE, data=module2trait)
        mod5 <- lm(L1HS ~ log2TPMsum + radiation + oldLINE, data=module2trait)
        mod6 <- lm(L1HS ~ modulePC1 + radiation + oldLINE, data=module2trait)
        mod7 <- lm(L1HS ~ modulePC1 + log2TPMsum + radiation + oldLINE, data=module2trait)
...
lm_module2oldLINE <- function(eigenGenes, traits, resultdir) {
  ...
  for(m in 1:length(modules)){
    ...
    if (cancertype == "THCA") {
      if (length(summary(module2trait$nested.batch))>1) {
        mod0 <- lm(oldLINE ~ garbage + L1HS, data=module2trait)
        mod1 <- lm(oldLINE ~ log2TPMsum + L1HS, data=module2trait)
        mod2 <- lm(oldLINE ~ modulePC1 + L1HS, data=module2trait)
        mod3 <- lm(oldLINE ~ modulePC1 + log2TPMsum + L1HS, data=module2trait)
        mod4 <- lm(oldLINE ~ nested.batch + L1HS, data=module2trait)
        mod5 <- lm(oldLINE ~ log2TPMsum + nested.batch + L1HS, data=module2trait)
        mod6 <- lm(oldLINE ~ modulePC1 + nested.batch + L1HS, data=module2trait)
        mod7 <- lm(oldLINE ~ modulePC1 + log2TPMsum + nested.batch + L1HS, data=module2trait)
        mod8 <- lm(oldLINE ~ radiation + L1HS, data=module2trait)
        mod9 <- lm(oldLINE ~ log2TPMsum + radiation + L1HS, data=module2trait)
        mod10 <- lm(oldLINE ~ modulePC1 + radiation + L1HS, data=module2trait)
        mod11 <- lm(oldLINE ~ modulePC1 + log2TPMsum + radiation + L1HS, data=module2trait)
        mod12 <- lm(oldLINE ~ nested.batch + radiation + L1HS, data=module2trait)
        mod13 <- lm(oldLINE ~ log2TPMsum + nested.batch + radiation + L1HS, data=module2trait)
        mod14 <- lm(oldLINE ~ modulePC1 + nested.batch + radiation + L1HS, data=module2trait)
        mod15 <- lm(oldLINE ~ modulePC1 + log2TPMsum + nested.batch + radiation + L1HS, data=module2trait)
        AICc<-model.sel(mod0,mod1,mod2,mod3,mod4, mod5, mod6, mod7,mod8,mod9,mod10,mod11,mod12,mod13,mod14,mod15)
      } else {
        mod0 <- lm(oldLINE ~ garbage + L1HS, data=module2trait)
        mod1 <- lm(oldLINE ~ log2TPMsum + L1HS, data=module2trait)
        mod2 <- lm(oldLINE ~ modulePC1 + L1HS, data=module2trait)
        mod3 <- lm(oldLINE ~ modulePC1 + log2TPMsum + L1HS, data=module2trait)
        mod4 <- lm(oldLINE ~ radiation + L1HS, data=module2trait)
        mod5 <- lm(oldLINE ~ log2TPMsum + radiation + L1HS, data=module2trait)
        mod6 <- lm(oldLINE ~ modulePC1 + radiation + L1HS, data=module2trait)
        mod7 <- lm(oldLINE ~ modulePC1 + log2TPMsum + radiation + L1HS, data=module2trait)
        AICc<-model.sel(mod0,mod1,mod2,mod3,mod4,mod5,mod6,mod7)
      }      
    } else {
    if (length(summary(module2trait$nested.batch))>1) {
