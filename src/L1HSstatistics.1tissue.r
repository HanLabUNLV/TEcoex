...
if (file.exists(paste0(resultdir,"ddsGenes.RData"))) {
  ...
  if (grepl("TET", genedatadir)) {
    genefile.names <- dir(genedatadir, pattern ="ele.cntTable", recursive=TRUE)
    genefile.names <- genefile.names[!grepl ("discount", genefile.names)]
    genefile.names <- genefile.names[!grepl ("v1", genefile.names)]
    
    tissueID <- data.frame(matrix(unlist(str_split(genefile.names, "/")), ncol=2, byrow=TRUE))
    ...
    genefile.names <- dir(genedatadir, pattern ="genes.results", recursive=TRUE)
    names <- data.frame(matrix(unlist(strsplit(genefile.names, "/")), ncol=2, byrow=TRUE))
    tissues <- factor(names[,1])
    patientIDs <- factor(substr(names[,2], 1, 12))
  }
  genefile.names = paste0(genedatadir,genefile.names)
  genepatient_files <- cbind.data.frame(patientIDs, genefile.names)
  colnames(genepatient_files) <- c("patient", "filename")
  

  TEfile.names <- dir(TEdatadir, pattern ="ele.cntTable", recursive=TRUE)
  TEfile.names <- TEfile.names[!grepl ("discount", TEfile.names)]
  TEfile.names <- TEfile.names[!grepl ("v1", TEfile.names)]
  
  TEtissueID <- data.frame(matrix(unlist(str_split(TEfile.names, "/")), ncol=2, byrow=TRUE))
  TEtissues <- factor(TEtissueID[,1])
  TEpatientIDs <- factor(data.frame(matrix(unlist(str_split(TEtissueID[,2], "[.]", n=2)), ncol=2, byrow=TRUE))[,1])
  
  patient_files = genepatient_files[TEpatientIDs,]
  patientIDs = TEpatientIDs
  tissues = TEtissues
  ...
  batchids <- read.table("data/barcodes.tissue.batch.txt", header=TRUE, sep="\t")
  rownames(batchids) <- batchids$patient
  batchids <- batchids[coldata$patient,]
  coldata <- cbind.data.frame(coldata, batchids[-c(2,3)])
  ...
  if (file.exists(paste0(resultdir,"sizefactors.1.txt"))) {
    sf <- read.table(paste0(resultdir,"sizefactors.1.txt"), row.names=1, sep="\t", header=TRUE) 
    ...
    write.table(sf, file=paste0(resultdir,"sizefactors.1.txt"), quote=FALSE, row.names=TRUE, sep="\t")
  }
  ...
  if (file.exists(paste0(resultdir,"ddsGenes.LRT.RData"))) {
    ...
    ddsGenes <- nbinomLRT(ddsGenes, reduced = ~ 1, maxit=1000)
  ...
  write.table(sizeFactors(ddsGenes), file=paste0(resultdir,"sizefactors.2.txt"), quote=FALSE, row.names=TRUE, sep="\t")
...
pheatmap(matrix, 
  ...
  if (file.exists(paste0(resultdir,"ddsNew.LRT.RData"))) {
    ...
    ddsnew <- nbinomLRT(ddsnew, reduced = ~ 1, maxit=2000)
