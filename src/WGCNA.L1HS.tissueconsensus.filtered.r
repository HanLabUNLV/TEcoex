...
workingDir = "..";
setwd(workingDir); 
# Load the WGCNA.filtered package
#library(DESeq2)
library(WGCNA);
...
dir.create(paste0(resultdir, "WGCNA.filtered")) 
dir.create(paste0(resultdir, "WGCNA.filtered/consensus/")) 
dir.create(paste0(resultdir, "WGCNA.filtered/consensus/Plots")) 


if (file.exists(paste0(resultdir, "WGCNA.filtered/consensus/Consensus-dataInput.RData"))) {
  load(paste0(resultdir, "WGCNA.filtered/consensus/Consensus-dataInput.RData")) 
  nSets = length(Traits);
...
normalcnts <- read.table(paste0(resultdir, "VSTcnt.noovp.1k.txt"), header=TRUE, row.names=1, sep="\t", check.names = FALSE)
...
write.table(traitData, paste0(resultdir, "WGCNA.filtered/", "WGCNA.filtered.L1HS.clinical.txt"), quote=FALSE, sep="\t", row.names=FALSE);
...
pdf(file = paste0(resultdir, "WGCNA.filtered/consensus/Plots/SampleClustering.pdf"), width = 12, height = 12);
par(mfrow=c(2,1))
par(mar = c(0, 4, 2, 0))
for (set in 1:nSets)
...
#traitData = read.table("WGCNA.filtered.L1HS.clinical.txt", header=TRUE, sep="\t");
dim(traitData)
names(traitData)
...
save(multiExpr, Traits, nGenes, nSamples, setLabels, shortLabels, exprSize, 
     file = paste0(resultdir, "WGCNA.filtered/consensus/Consensus-dataInput.RData"));
...
if (file.exists(paste0(resultdir, "WGCNA.filtered/consensus/Consensus-NetworkConstruction-auto.RData"))) {
  load(paste0(resultdir, "WGCNA.filtered/consensus/Consensus-NetworkConstruction-auto.RData"))
} else {
...
#sizeGrWindow(8, 6)
pdf(file = paste0(resultdir, "WGCNA.filtered/consensus/Plots/scaleFreeAnalysis.pdf"), wi = 8, he = 6)
par(mfcol = c(2,2));
...
net = blockwiseConsensusModules(
        ...
        minKMEtoStay = 0,
        saveTOMs = TRUE, 
        saveTOMFileBase = paste0(resultdir, "WGCNA.filtered/consensus/TCGA.genes.TOM"), 
        verbose = 5)
...
save(net, consMEs, moduleLabels, moduleColors, consTree, file = paste0(resultdir, "WGCNA.filtered/consensus/Consensus-NetworkConstruction-auto.RData"))
...
#sizeGrWindow(8,6);
pdf(file = paste0(resultdir, "WGCNA.filtered/consensus/Plots/ConsensusDendrogram-auto.pdf"), wi = 16, he = 6)
...
# linear model with covariate
  ...
  module2L1HS = lm_module2trait(consMEs[[set]]$data, traitmat, paste0(resultdir, "WGCNA.filtered/consensus/"), "L1HS.5prime")
  module2oldLINE = lm_module2trait(consMEs[[set]]$data, traitmat,  paste0(resultdir, "WGCNA.filtered/consensus/"), "oldLINE")
  module2gender = lm_trait2module(consMEs[[set]]$data, traitmat,  paste0(resultdir, "WGCNA.filtered/consensus/"), "gender", TRUE)
  module2age = lm_trait2module(consMEs[[set]]$data, traitmat,  paste0(resultdir, "WGCNA.filtered/consensus/"), "age", FALSE)
...
#sizeGrWindow(10,7)
pdf(file = paste0(resultdir, "WGCNA.filtered/consensus/Plots/ModuleTraitRelationships-",shortLabels[set],".pdf"), wi = 10, he = 12);
...
#sizeGrWindow(10,7)
pdf(file = paste0(resultdir, "WGCNA.filtered/consensus/Plots/ModuleTraitRelationships-consensus.pdf"), wi = 10, he = 12);
par(mar = c(6, 8.8, 3, 2.2));
...
pdf(file = paste0(resultdir, "WGCNA.filtered/consensus/Plots/EigengeneNetworks.pdf"), width= 12, height = 12);
par(cex = 0.9)
...
# plot heatmap
pdf(file = paste0(resultdir, "WGCNA.filtered/consensus/Plots/EigengeneHeatmaps.pdf"), width= 12, height = 12);
for (set in 1:nSets) {
...
for (set in 1:nSets) {
  ...
  write.table(eigenGenes, paste0(resultdir, "WGCNA.filtered/consensus/", paste0("moduleEigengenes.",setLabels[[set]],".txt")), quote=FALSE, row.names=TRUE, sep="\t")
}
...
info.GS = data.frame(Gene = genenames, GeneSymbol = annot$GENE[gene2annot],
             ...
             GSsmall);
write.csv(info.GS, file = paste0(resultdir, "WGCNA.filtered/consensus/small.GS.csv"), row.names = FALSE, quote = FALSE);
...
info.kME.cor = data.frame(Gene = genenames, GeneSymbol = annot$GENE[gene2annot],
             ...
             ModuleColor = labels2colors(moduleLabels),
             kMEcor);
write.csv(info.kME, file = paste0(resultdir, "WGCNA.filtered/consensus/small.kMEmat.csv"), row.names = FALSE, quote = FALSE);
write.csv(info.kME.cor, file = paste0(resultdir, "WGCNA.filtered/consensus/kME.membership.csv"), row.names = FALSE, quote = FALSE);
...
for( i in TEmodules) {
  ...
  for (set in consistenttissues) {
    ...
    write.table(info.kME.cor[FilterGenes, 1:5], file = paste0(resultdir, "WGCNA.filtered/consensus/TEmodule.", i, ".", setLabels[set], ".highMMgenes.txt"), row.names = FALSE, quote = FALSE)

    FilterGenes= kMEcor[,collabel]>.6
    importantgenes = importantgenes & FilterGenes
  }
  write.table(info.kME.cor[importantgenes, 1:5], file = paste0(resultdir, "WGCNA.filtered/consensus/TEmodule.", i, ".consensus.highMMgenes.txt"), row.names = FALSE, quote = FALSE)
  ...
  write.table(TSS.importantgenes[,c(3, 6, 7, 1, 8, 5)], file = paste0(resultdir, "WGCNA.filtered/consensus/TEmodule.", i, ".TSS.bed"), row.names = FALSE, quote = FALSE, sep="\t")

}
plotGeneIdx.pos = plotGeneIdx
write.table(info.kME.cor[plotGeneIdx.pos, 1:5], file = paste0(resultdir, "WGCNA.filtered/consensus/labeltree.pos.txt"), row.names = FALSE, quote = FALSE, sep="\t")
...
for( i in negTEmodules) {
  ...
  for (set in consistenttissues) {
    collabel = paste0("kME.set", set, "ME", i)
    FilterGenes= kMEcor[,collabel]>.8
    write.table(info.kME.cor[FilterGenes, 1:5], file = paste0(resultdir, "WGCNA.filtered/consensus/negTEmodule.", i, ".", setLabels[set], ".highMMgenes.txt"), row.names = FALSE, quote = FALSE)

    FilterGenes= kMEcor[,collabel]>.6
    importantgenes = importantgenes & FilterGenes
  }
  write.table(info.kME.cor[importantgenes, 1:5], file = paste0(resultdir, "WGCNA.filtered/consensus/negTEmodule.", i, ".consensus.highMMgenes.txt"), row.names = FALSE, quote = FALSE)
  ...
  TSS.importantgenes$end = TSS.importantgenes[,4]+1000
  TSS.importantgenes$score = rep(".", nrow(TSS.importantgenes))
  write.table(TSS.importantgenes[,c(3, 6, 7, 1, 8, 5)], file = paste0(resultdir, "WGCNA.filtered/consensus/negTEmodule.", i, ".TSS.bed"), row.names = FALSE, quote = FALSE, sep="\t")


}
plotGeneIdx.neg = plotGeneIdx
plotGeneIdx.neg[plotGeneIdx.pos] = FALSE
write.table(info.kME.cor[plotGeneIdx.neg, 1:5], file = paste0(resultdir, "WGCNA.filtered/consensus/labeltree.neg.txt"), row.names = FALSE, quote = FALSE, sep="\t")


pdf(file = paste0(resultdir, "WGCNA.filtered/consensus/Plots/heatmap.pdf"), width = 12, height = 12);
power=6
color1=moduleColors
...
for (set in 1:nSets)
{
  ...
  #write.table(labeltree, file = paste0(resultdir, "WGCNA.filtered/consensus/heatmap.labeltree.", setLabels[set], ".txt"))
  write.table(labelrow, file = paste0(resultdir, "WGCNA.filtered/consensus/heatmap.labelrow.", setLabels[set], ".txt"))
...
for( i in modulenumbers) {
  ...
  reportfileName=paste0(resultdir, "WGCNA.filtered/consensus/", paste("termClusterReport",i,labels2colors(i),"txt",sep="."))
  ...
  if (length(members(termCluster)) > 0) {
    ...
    lapply(symbols, write, paste0(resultdir, "WGCNA.filtered/consensus/", paste("termClusterSymbols",i,labels2colors(i),"txt", sep=".")), append=TRUE, ncolumns=1000)
...
for( i in modulenumbers) {
  if (i == 0) {print ("skipping grey"); next;}
  reportFileName=paste0(resultdir, "WGCNA.filtered/consensus/", paste("Reactome",i,labels2colors(i),"txt",sep="."))
  if (file.exists(reportFileName)) {print (paste0("skipping ", reportFileName)); next}
  genelist <- info.kME[info.kME$ModuleLabel==i,"EntrezID"]
  genelist = genelist[!is.na(genelist)]
  x <- enrichPathway(gene=genelist, pvalueCutoff=0.05, readable=T)
  if (! is.null(x) && nrow(x)) {
    write.table(as.data.frame(x), file = paste0(resultdir, "WGCNA.filtered/consensus/", paste("Reactome",i,labels2colors(i),"txt", sep=".") ), row.names = FALSE, quote = FALSE);
    dotplot(x, showCategory=15)
    ggsave(filename=paste0(resultdir, "WGCNA.filtered/consensus/Plots/", paste("Reactome",i,labels2colors(i),"pdf", sep=".")), width=12, height=(min(nrow(as.data.frame(x)),15)*0.32), units = "in", limitsize = FALSE )
    emapplot(x)
    ggsave(filename=paste0(resultdir, "WGCNA.filtered/consensus/Plots/", paste("Reactome",i,labels2colors(i),"em.pdf", sep=".")), width=8, height=12, units = "in")
  }
}
...
pdf(file = paste0(resultdir, "WGCNA.filtered/consensus/Plots/TOMplot.pdf"), width = 12, height = 12);
power=6
color1=moduleColors
  restGenes= (color1 != "grey")
  restGenes = genesinTEmodules
