...
#cancertypes = list("BLCA", "BRCA", c("COAD", "READ"), c("ESCA", "STAD"), "HNSC", "KICH", "KIRC", "KIRP", "LIHC", "LUAD", "LUSC", "PRAD", "THCA", "UCEC")
...
for (j in 1:length(cancertypes)) {
  ...
  for(i in 1:length(genenames)){
    ...
    test<-cor.test(dataratio$gene_n,dataratio$oldLINEVST, method="pearson")
    pearson_r[i] <- test$estimate
    pearson_pval[i] <- test$p.value
    test<-cor.test(dataratio$gene_n,dataratio$oldLINEVST, method="spearman")
    spearman_rho[i] <- test$estimate
    spearman_pval[i] <- test$p.value

    AICc = NULL    
    set.seed(0)
    garbage <- rnorm(length(dataratio$oldLINEVST))  
    if (cancertype == "THCA") {
      if (length(summary(dataratio$nested.batch))>1) {
        mod0 <- lm(oldLINEVST ~ garbage + VSTcnts, data=dataratio)
        mod1 <- lm(oldLINEVST ~ log2TPMsum + VSTcnts, data=dataratio)
        mod2 <- lm(oldLINEVST ~ gene_n + VSTcnts, data=dataratio)
        mod3 <- lm(oldLINEVST ~ gene_n + log2TPMsum + VSTcnts, data=dataratio)
        mod4 <- lm(oldLINEVST ~ nested.batch + VSTcnts, data=dataratio)
        mod5 <- lm(oldLINEVST ~ log2TPMsum + nested.batch + VSTcnts, data=dataratio)
        mod6 <- lm(oldLINEVST ~ gene_n + nested.batch + VSTcnts, data=dataratio)
        mod7 <- lm(oldLINEVST ~ gene_n + log2TPMsum + nested.batch + VSTcnts, data=dataratio)
        mod8 <- lm(oldLINEVST ~ THCAradiation + VSTcnts, data=dataratio)
        mod9 <- lm(oldLINEVST ~ log2TPMsum + THCAradiation + VSTcnts, data=dataratio)
        mod10 <- lm(oldLINEVST ~ gene_n + THCAradiation + VSTcnts, data=dataratio)
        mod11 <- lm(oldLINEVST ~ gene_n + log2TPMsum + THCAradiation + VSTcnts, data=dataratio)
        mod12 <- lm(oldLINEVST ~ nested.batch + THCAradiation + VSTcnts, data=dataratio)
        mod13 <- lm(oldLINEVST ~ log2TPMsum + nested.batch + THCAradiation + VSTcnts, data=dataratio)
        mod14 <- lm(oldLINEVST ~ gene_n + nested.batch + THCAradiation + VSTcnts, data=dataratio)
        mod15 <- lm(oldLINEVST ~ gene_n + log2TPMsum + nested.batch + THCAradiation + VSTcnts, data=dataratio)
        ...
        mod2 <- lm(oldLINEVST ~ gene_n + VSTcnts, data=dataratio)
        mod3 <- lm(oldLINEVST ~ gene_n + log2TPMsum + VSTcnts, data=dataratio)
        mod4 <- lm(oldLINEVST ~ THCAradiation + VSTcnts, data=dataratio)
        mod5 <- lm(oldLINEVST ~ log2TPMsum + THCAradiation + VSTcnts, data=dataratio)
        mod6 <- lm(oldLINEVST ~ gene_n + THCAradiation + VSTcnts, data=dataratio)
        mod7 <- lm(oldLINEVST ~ gene_n + log2TPMsum + THCAradiation + VSTcnts, data=dataratio)
  ...
  filename = paste(resultdir, subdir, cancerdir, "results_cor.txt", sep="/")
  ...
  filename = paste(resultdir, subdir, cancerdir, "results_lm.txt", sep="/")
...
for (j in 1:length(cancertypes)) {
  type <- cancertypes[j]
  cancerdir <- cancerdirs[j]
  fname = paste(resultdir, subdir,cancerdir, "results_positive_lm.txt", sep="/")
...
write.table(coexpressed_plus, file=paste(resultdir, subdir, "coexpressed_plus.txt", sep="/"), sep="\t", quote = FALSE, row.names = FALSE )
write.table(coexpressed_minus, file=paste(resultdir, subdir, "coexpressed_minus.txt", sep="/"), sep="\t", quote = FALSE, row.names = FALSE )
